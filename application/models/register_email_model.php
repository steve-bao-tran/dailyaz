...
# * @Copyright: 2018 - 2020              #
#****************************************#
class Register_email_model extends CI_Model
{
	function __construct()
	{
		parent::__construct();
		$this->table_name = "registry_email";
	}
	...
	function fetch($select = "*", $where = "", $order = "re_id", $by = "DESC", $start = -1, $limit = 0)
	{
        $this->db->cache_off();
		$this->db->select($select);
		if($where && $where != "")
	...
	function fetch_join1($select = "*", $join, $table, $on, $where = "", $order = "re_id", $by = "DESC", $start = -1, $limit = 0, $distinct = false)
	{
        $this->db->cache_off();
	...
	function fetch_join2($select = "*", $join_1, $table_1, $on_1, $join_2, $table_2, $on_2, $wherestore = "", $order = "re_id", $by = "DESC", $start = -1, $limit = 0, $distinct = false, $group_by=NULL)
    {
        $this->db->cache_off();
        $this->db->select($select);
    ...
    function fetch_join3($select = "*", $join_1, $table_1, $on_1, $join_2, $table_2, $on_2, $join_3, $table_3, $on_3, $wherestore = "", $order = "re_id", $by = "DESC", $start = -1, $limit = 0, $distinct = false, $group=NULL)
    {
        $this->db->cache_off();
        $this->db->select($select);
        if ($join_1 && ($join_1 == "INNER" || $join_1 == "LEFT" || $join_1 == "RIGHT") && $table_1 && $table_1 != "" && $on_1 && $on_1 != "") {
	...
	function delete($value, $field = "re_id", $in = true)
    {
		if($in == true)
		{
